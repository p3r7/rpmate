...
TimberEq.sample_changed_callback = function() end
TimberEq.meta_changed_callback = function() end
TimberEq.waveform_changed_callback = function() end
TimberEq.play_positions_changed_callback = function() end
TimberEq.views_changed_callback = function() end

TimberEq.setup_params_dirty = false
TimberEq.filter_dirty = false
TimberEq.env_dirty = false
TimberEq.lfo_functions_dirty = false
TimberEq.lfo_1_dirty = false
TimberEq.lfo_2_dirty = false
TimberEq.bpm = 120
TimberEq.display = "id" -- Can be "id", "note" or "none"
TimberEq.shift_mode = false
TimberEq.file_select_active = false
...
TimberEq.specs = specs
TimberEq.options = options
TimberEq.samples_meta = samples_meta
TimberEq.num_sample_params = 0
...
local function update_freq_multiplier(sample_id)
  ...
  elseif scale_by == 3 then
    multiplier = sample_duration / (options.BY_BARS_DECIMAL[params:get("by_bars_" .. sample_id)] * (60 / TimberEq.bpm * 4))
  end
...
local function update_by_bar_multipliers()
  for i = 0, TimberEq.num_sample_params - 1 do
...
function TimberEq.load_sample(id, file)
...
local function sample_loaded(id, streaming, num_frames, num_channels, sample_rate)
  ...
  TimberEq.sample_changed_callback(id)
  TimberEq.meta_changed_callback(id)
  TimberEq.waveform_changed_callback(id)
  TimberEq.play_positions_changed_callback(id)
...
function TimberEq.clear_samples(first, last)
  first = first or 0
  last = last or first
  ...
  for i = first, last do
    ...
    TimberEq.meta_changed_callback(i)
    TimberEq.waveform_changed_callback(i)
    TimberEq.play_positions_changed_callback(i)
  end

  TimberEq.views_changed_callback(nil)
  TimberEq.setup_params_dirty = true
end

function TimberEq.request_waveform(sample_id)
...
local function move_copy_update(from_id, to_id)
  ...
  TimberEq.setup_params_dirty = true
  TimberEq.filter_dirty = true
  TimberEq.env_dirty = true
  TimberEq.lfo_functions_dirty = true
  TimberEq.views_changed_callback(nil)
end

function TimberEq.move_sample(from_id, to_id)
...
function TimberEq.copy_sample(from_id, to_first_id, to_last_id)
...
function TimberEq.copy_params(from_id, to_first_id, to_last_id, param_ids)
...
function TimberEq.osc_event(path, args, from)
...
osc.event = TimberEq.osc_event
-- NOTE: If you need the OSC callback in your script then TimberEq.osc_event(path, args, from)
-- must be called from the end of that function to pass the data down to this lib

function TimberEq.set_bpm(bpm)
  TimberEq.bpm = bpm
...
local function format_hide_for_stream(sample_id, param_name, formatter)
  return function(param)
    if TimberEq.samples_meta[sample_id].streaming == 1 then
      return "N/A"
    else
      if formatter then
...
function TimberEq.add_params()

  params:add_separator("TimberEq")

  params:add{type = "trigger", id = "clear_all", name = "Clear All", action = function(value)
    TimberEq.clear_samples(0, #samples_meta - 1)
  end}
...
function TimberEq.add_sample_params(id, include_beat_params, extra_params)
  ...
  TimberEq.num_sample_params = TimberEq.num_sample_params + 1
end
...
TimberEq.UI = {}
TimberEq.UI.__index = TimberEq.UI

function TimberEq.draw_title(sample_id, show_sample_name)
  if show_sample_name == nil then show_sample_name = true end

  screen.level(15)

  if TimberEq.shift_mode then
    screen.rect(0, 4, 1, 5)
    screen.fill()
  end
  ...
  screen.move(4, 9)
  if TimberEq.display == "id" then
    screen.text(string.format("%03d", sample_id))
    screen.move(23, 9)
  elseif TimberEq.display == "note" then
    screen.text(MusicUtil.note_num_to_name(sample_id, true))
    screen.move(27, 9)
    max_title_width = 96
  end

  if show_sample_name or TimberEq.shift_mode then
    local title
...
TimberEq.UI.SampleSetup = {}
TimberEq.UI.SampleSetup.__index = TimberEq.UI.SampleSetup
...
local function update_setup_params(self)
  ...
  TimberEq.setup_params_dirty = false
  TimberEq.views_changed_callback(self.sample_id)
end

function TimberEq.UI.SampleSetup.new(sample_id, index)
  ...
  setmetatable(TimberEq.UI.SampleSetup, {__index = TimberEq.UI})
  setmetatable(sample_setup, TimberEq.UI.SampleSetup)
...
function TimberEq.UI.SampleSetup:set_sample_id(id)
  self.sample_id = id
  TimberEq.setup_params_dirty = true
...
function TimberEq.UI.SampleSetup:set_index(index)
...
function TimberEq.UI.SampleSetup:set_param_default()
...
function TimberEq.UI.SampleSetup:set_param_delta(delta)
  if self.selected_param_name ~= "" then
...
function TimberEq.UI.SampleSetup:enc(n, delta)

  if self.move_active then
    if n == 2 or n == 3 then
      self.move_to = util.clamp(util.round(self.move_to + delta), 0, TimberEq.num_sample_params - 1)
    end

  elseif self.copy_active or self.copy_params_active then
    if n == 2 then
      self.copy_to_first = util.clamp(util.round(self.copy_to_first + delta), 0, TimberEq.num_sample_params - 1)
      self.copy_to_last = util.clamp(self.copy_to_last, self.copy_to_first, TimberEq.num_sample_params - 1)
    elseif n == 3 then
      self.copy_to_last = util.clamp(util.round(self.copy_to_last + delta), self.copy_to_first, TimberEq.num_sample_params - 1)
    end
...
function TimberEq.UI.SampleSetup:key(n, z)
  if z == 1 then
    ...
    if self.move_active then
      ...
      elseif n == 3 then
        TimberEq.move_sample(self.sample_id, self.move_to)
        self.move_active = false
        self.move_to = 0
        TimberEq.views_changed_callback(self.sample_id)
      end
    ...
    elseif self.copy_active then
      ...
      elseif n == 3 then
        TimberEq.copy_sample(self.sample_id, self.copy_to_first, self.copy_to_last)
        self.copy_active = false
        self.copy_to_first = 0
        self.copy_to_last = 0
        TimberEq.views_changed_callback(self.sample_id)
    ...
    elseif self.copy_params_active then
      ...
      elseif n == 3 then
        TimberEq.copy_params(self.sample_id, self.copy_to_first, self.copy_to_last, {})
        self.copy_params_active = false
        self.copy_to_first = 0
        self.copy_to_last = 0
        TimberEq.views_changed_callback(self.sample_id)
      ...
      if n == 3 then
        ...
        if self.index == 1 then
          TimberEq.file_select_active = true
          TimberEq.FileSelect.enter(_path.audio, function(file)
            TimberEq.file_select_active = false
            TimberEq.views_changed_callback(self.sample_id)
            if file ~= "cancel" then
              TimberEq.load_sample(self.sample_id, file)
            end
          end)
...
function TimberEq.UI.SampleSetup:sample_key(sample_id)

  if self.move_active then
    TimberEq.move_sample(self.sample_id, sample_id)
    self.move_active = false
    self.move_to = 0
    TimberEq.views_changed_callback(self.sample_id)

  elseif self.copy_active then
    TimberEq.copy_sample(self.sample_id, sample_id, sample_id)
    self.copy_active = false
    self.copy_to_first = 0
    self.copy_to_last = 0
    TimberEq.views_changed_callback(self.sample_id)
  ...
  elseif self.copy_params_active then
    TimberEq.copy_params(self.sample_id, sample_id, sample_id)
    self.copy_active = false
    self.copy_to_first = 0
    self.copy_to_last = 0
    TimberEq.views_changed_callback(self.sample_id)
...
function TimberEq.UI.SampleSetup:redraw()

  if TimberEq.setup_params_dirty then
    update_setup_params(self)
    self.selected_param_name = self.param_names[self.index]
  end

  TimberEq.draw_title(self.sample_id)
  local info = ""
...
TimberEq.UI.Waveform = {}
TimberEq.UI.Waveform.__index = TimberEq.UI.Waveform

function TimberEq.UI.Waveform.new(sample_id)
  local waveform = {
    sample_id = sample_id or 1,
    tab_id = 1,
  ...
  setmetatable(TimberEq.UI.Waveform, {__index = TimberEq.UI})
  setmetatable(waveform, TimberEq.UI.Waveform)
  return waveform
end

function TimberEq.UI.Waveform:set_sample_id(id)
  self.sample_id = id
  TimberEq.views_changed_callback(id)
end

function TimberEq.UI.Waveform:set_tab(id)
  self.tab_id = util.clamp(id, 1, 2)
end

function TimberEq.UI.Waveform:enc(n, delta)
...
function TimberEq.UI.Waveform:key(n, z)
  if z == 1 then
    if n == 2 then
    ...
    elseif n == 3 then
      ...
      if not samples_meta[self.sample_id].waveform_requested then
        TimberEq.request_waveform(self.sample_id)
...
function TimberEq.UI.Waveform:update()
...
function TimberEq.UI.Waveform:redraw()
  local X = 4
  local Y = 25
  local W = 120
  local H = 35
...
TimberEq.UI.FilterAmp = {}
TimberEq.UI.FilterAmp.__index = TimberEq.UI.FilterAmp
...
function TimberEq.UI.FilterAmp.new(sample_id, tab_id)
  ...
  setmetatable(TimberEq.UI.FilterAmp, {__index = TimberEq.UI})
  setmetatable(filter_amp, TimberEq.UI.FilterAmp)
  return filter_amp
end

function TimberEq.UI.FilterAmp:set_sample_id(id)
  self.sample_id = id
  TimberEq.filter_dirty = true
  TimberEq.views_changed_callback(id)
end

function TimberEq.UI.FilterAmp:set_tab(id)
  self.tab_id = util.clamp(id, 1, 2)
...
function TimberEq.UI.FilterAmp:enc(n, delta)
  if TimberEq.shift_mode then delta = delta * 0.1 end
  if self.tab_id == 1 then
    if n == 2 then
...
function TimberEq.UI.FilterAmp:key(n, z)
  if z == 1 then
    if n == 2 then
      self:set_tab(self.tab_id % 2 + 1)
    elseif n == 3 then
      if self.tab_id == 1 then
        params:set("filter_type_" .. self.sample_id, params:get("filter_type_" .. self.sample_id) % #TimberEq.options.FILTER_TYPE + 1)
      end
    end
    TimberEq.views_changed_callback(self.sample_id)
  end
...
function TimberEq.UI.FilterAmp:redraw()

  TimberEq.draw_title(self.sample_id)

  if TimberEq.filter_dirty then
...
TimberEq.UI.Env = {}
TimberEq.UI.Env.__index = TimberEq.UI.Env

function TimberEq.UI.Env.new(env_name, sample_id, tab_id)
  ...
  setmetatable(TimberEq.UI.Env, {__index = TimberEq.UI})
  setmetatable(env, TimberEq.UI.Env)
  return env
end

function TimberEq.UI.Env:set_sample_id(id)
  self.sample_id = id
  TimberEq.env_dirty = true
  TimberEq.views_changed_callback(id)
end

function TimberEq.UI.Env:set_tab(id)
  self.tab_id = util.clamp(id, 1, 2)
end

function TimberEq.UI.Env:enc(n, delta)

end

function TimberEq.UI.Env:key(n, z)
  if z == 1 then
    if n == 2 then
      self:set_tab(self.tab_id % 2 + 1)
    end
...
function TimberEq.UI.Env:redraw()
...
TimberEq.UI.AmpEnv = {}
TimberEq.UI.AmpEnv.__index = TimberEq.UI.AmpEnv

function TimberEq.UI.AmpEnv.new(sample_id, tab_id)
  local env = TimberEq.UI.Env.new("amp", sample_id, tab_id)
  setmetatable(TimberEq.UI.AmpEnv, {__index = TimberEq.UI.Env})
  setmetatable(env, TimberEq.UI.AmpEnv)
  return env
end

function TimberEq.UI.AmpEnv:enc(n, delta)
  if TimberEq.shift_mode then delta = delta * 0.1 end
  if self.tab_id == 1 then
    if n == 2 then
...
TimberEq.UI.ModEnv = {}
TimberEq.UI.ModEnv.__index = TimberEq.UI.ModEnv

function TimberEq.UI.ModEnv.new(sample_id, tab_id)
  local env = TimberEq.UI.Env.new("mod", sample_id, tab_id)
  setmetatable(TimberEq.UI.ModEnv, {__index = TimberEq.UI.Env})
  setmetatable(env, TimberEq.UI.ModEnv)
  return env
end

function TimberEq.UI.ModEnv:enc(n, delta)
  if TimberEq.shift_mode then delta = delta * 0.1 end
  if self.tab_id == 1 then
    if n == 2 then
...
TimberEq.UI.Lfos = {}
TimberEq.UI.Lfos.__index = TimberEq.UI.Lfos
...
local function generate_lfo_wave(sample_id, lfo_id)
  ...
  return function(x)
    ...
    if fade > 0 then
      fade_end = util.linlin(0, TimberEq.specs.LFO_FADE.maxval, 0, 1, fade)
      y_fade = util.linlin(0, fade_end, MIN_Y, 1, x)
    else
      fade_end = util.linlin(TimberEq.specs.LFO_FADE.minval, 0, 0, 1, fade)
      y_fade = util.linlin(0, fade_end, 1, util.linlin(TimberEq.specs.LFO_FADE.minval * 0.2, 0, MIN_Y, 1, fade), x)
    end

    x = x * util.linlin(TimberEq.specs.LFO_1_FREQ.minval, TimberEq.specs.LFO_1_FREQ.maxval, 0.5, 10, freq)
    local y
...
function TimberEq.UI.Lfos.new(sample_id, tab_id)
  ...
  setmetatable(TimberEq.UI.Lfos, {__index = TimberEq.UI})
  setmetatable(lfos, TimberEq.UI.Lfos)
  return lfos
end

function TimberEq.UI.Lfos:set_sample_id(id)
  self.sample_id = id
  TimberEq.lfo_functions_dirty = true
  TimberEq.views_changed_callback(id)
end

function TimberEq.UI.Lfos:set_tab(id)
  self.tab_id = util.clamp(id, 1, 2)
  self.lfo_1_graph:set_active(self.tab_id == 1)
  self.lfo_2_graph:set_active(self.tab_id == 2)
end

function TimberEq.UI.Lfos:enc(n, delta)
  if TimberEq.shift_mode then delta = delta * 0.05 end
  if self.tab_id == 1 then
    if n == 2 then
...
function TimberEq.UI.Lfos:key(n, z)
  if z == 1 then
    if n == 2 then
      self:set_tab(self.tab_id % 2 + 1)
    elseif n == 3 then
      if self.tab_id == 1 then
        params:set("lfo_1_wave_shape", params:get("lfo_1_wave_shape") % #TimberEq.options.LFO_WAVE_SHAPE + 1)
      else
        params:set("lfo_2_wave_shape", params:get("lfo_2_wave_shape") % #TimberEq.options.LFO_WAVE_SHAPE + 1)
      end
    end
    TimberEq.views_changed_callback(self.sample_id)
  end
...
function TimberEq.UI.Lfos:update()
...
function TimberEq.UI.Lfos:redraw()

  TimberEq.draw_title(self.sample_id)

  if TimberEq.lfo_functions_dirty then
  ...
  if TimberEq.lfo_1_dirty then
    self.lfo_1_graph:update_functions()
    TimberEq.lfo_1_dirty = false
  end
  if TimberEq.lfo_2_dirty then
...
local function draw_matrix(cols, rows, data, index, shift_mode)
  ...
  if not TimberEq.shift_mode then
    for i = 1, #cols do
...
TimberEq.UI.ModMatrix = {}
TimberEq.UI.ModMatrix.__index = TimberEq.UI.ModMatrix

function TimberEq.UI.ModMatrix.new(sample_id, index)
  local matrix = {
    sample_id = sample_id or 1,
    index = index or 1
  }
  setmetatable(TimberEq.UI.ModMatrix, {__index = TimberEq.UI})
  setmetatable(matrix, TimberEq.UI.ModMatrix)
  return matrix
end

function TimberEq.UI.ModMatrix:set_sample_id(id)
  self.sample_id = id
  TimberEq.views_changed_callback(id)
end

function TimberEq.UI.ModMatrix:set_index(index)
  self.index = util.clamp(index, 1, 11)
end

function TimberEq.UI.ModMatrix:enc(n, delta)
  if n == 2 then
    self:set_index(self.index + delta)
  elseif n == 3 then
    if TimberEq.shift_mode then delta = delta * 0.1 end
...
function TimberEq.UI.ModMatrix:key(n, z)
  if n == 3 and z == 1 then
    if self.index == 1 then
...
function TimberEq.UI.ModMatrix:redraw()

  TimberEq.draw_title(self.sample_id, false)
...
TimberEq.UI.KeyMatrix = {}
TimberEq.UI.KeyMatrix.__index = TimberEq.UI.KeyMatrix

function TimberEq.UI.KeyMatrix.new(sample_id, index)
  local matrix = {
    sample_id = sample_id or 1,
    index = index or 1
  }
  setmetatable(TimberEq.UI.KeyMatrix, {__index = TimberEq.UI})
  setmetatable(matrix, TimberEq.UI.KeyMatrix)
  return matrix
end

function TimberEq.UI.KeyMatrix:set_sample_id(id)
  self.sample_id = id
  TimberEq.views_changed_callback(id)
end

function TimberEq.UI.KeyMatrix:set_index(index)
  self.index = util.clamp(index, 1, 3)
end

function TimberEq.UI.KeyMatrix:enc(n, delta)
  if n == 2 then
    self:set_index(self.index + delta)
  elseif n == 3 then
    if TimberEq.shift_mode then delta = delta * 0.1 end
...
function TimberEq.UI.KeyMatrix:key(n, z)
  if n == 3 and z == 1 then
    if self.index == 1 then
...
function TimberEq.UI.KeyMatrix:redraw()
